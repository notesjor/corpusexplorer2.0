...
{
  public class ImporterKorap2021Mini : ImporterKorap
  {
      ...
      using (var zip = LoadStrategy.Initialize(path))
      {
        ...
        Parallel.ForEach(tDatas, Configuration.ParallelOptions, tData =>
        {
          try
          {
            ...
            // Add Metadata
            AddDocumentMetadata(dsel, new Dictionary<string, object> { { "ZipPath", textRoot } });

            var entryPath = FindEntryPath(zip, textRoot);

            if (!BuildWordLayer(path, zip, entryPath, tData, textRoot, dsel, out var skeleton,
                  out var references))
              return;
    ...
    private bool BuildWordLayer(string path, AbstractKorapLoadStrategy zip, string zipPath, string tData,
      string textRoot, Guid dsel, out int[] skelOut, out Dictionary<int, int[][]> refsOut)
    {
      string currentText;
      try
      {
        var res = zip.Read(tData).DocumentNode.SelectSingleNode("//text").InnerText;
        //for (var i = 0; i < 1; i++) // HtmlDecodeRounds = 1
