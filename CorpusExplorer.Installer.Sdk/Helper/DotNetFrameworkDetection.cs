    ...
    private static IEnumerable<string> FindMsBuild()
    {
      ...
      try
      {
        if (Directory.Exists(@"C:\Windows\Microsoft.NET\Framework\"))
          res.AddRange(Directory.GetFiles(@"C:\Windows\Microsoft.NET\Framework\", "MSBuild.exe", SearchOption.AllDirectories));
      }
      catch
      {
        // ignore
      ...
      try
      {
        if (Directory.Exists(@"C:\Windows\Microsoft.NET\Framework64"))
          res.AddRange(Directory.GetFiles(@"C:\Windows\Microsoft.NET\Framework64", "MSBuild.exe", SearchOption.AllDirectories));
      }
      catch
      {
        // ignore
