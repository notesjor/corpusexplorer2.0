...
namespace CorpusExplorer.Sdk.Utils.DocumentProcessing.Cluster
{
  ...
  {
    public static ConcurrentQueue<Dictionary<string, object>> Build(ref Queue<string> files, AbstractScraper scraper,
                                                                    int clusterSize, out List<string> processedFiles)
    {
    ...
    public static ConcurrentQueue<ConcurrentQueue<Dictionary<string, object>>> Build(
      ref List<Dictionary<string, object>> input, int clusterSize)
    {
    ...
    public static ConcurrentQueue<ConcurrentQueue<Dictionary<string, object>>> Build(
      ref Queue<Dictionary<string, object>> input, int clusterSize)
    {
      var local = (IEnumerable<Dictionary<string, object>>) input;
      return Build(ref local, clusterSize,
                   x =>
    ...
    public static ConcurrentQueue<ConcurrentQueue<Dictionary<string, object>>> Build(
      ref ConcurrentQueue<Dictionary<string, object>> input, int clusterSize)
    {
      var local = (IEnumerable<Dictionary<string, object>>) input;
      return Build(ref local, clusterSize,
                   x =>
