...
namespace CorpusExplorer.Sdk.Model.CorpusExplorer
{
  ...
  public sealed class CeTriple<TA, TB, TC>
  {
    ...
    [NonSerialized] private readonly Dictionary<TA, int> _dicA = new Dictionary<TA, int>();

    [NonSerialized] private readonly Dictionary<TB, int> _dicB = new Dictionary<TB, int>();

    [NonSerialized] private readonly Dictionary<TC, int> _dicC = new Dictionary<TC, int>();
    ...
    public IEnumerable<TA> RequestAllAs(TB b)
    {
      return Request(1, _dicB[b], x => _resA[x[0]]);
    }

    public IEnumerable<TA> RequestAllAs(TC c)
    {
      return Request(2, _dicC[c], x => _resA[x[0]]);
    }

    public IEnumerable<TB> RequestAllBs(TA a)
    {
      return Request(0, _dicA[a], x => _resB[x[1]]);
    }

    public IEnumerable<TB> RequestAllBs(TC c)
    {
      return Request(2, _dicC[c], x => _resB[x[1]]);
    }

    public IEnumerable<TC> RequestAllCs(TA a)
    {
      return Request(0, _dicA[a], x => _resC[x[2]]);
    }

    public IEnumerable<TC> RequestAllCs(TB b)
    {
      return Request(1, _dicB[b], x => _resC[x[2]]);
    }
