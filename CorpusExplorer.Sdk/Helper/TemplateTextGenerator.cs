...
namespace CorpusExplorer.Sdk.Helper
{
  public static class TemplateTextGenerator
  {
    ...
    public static string GenerateFromFile(string path, Dictionary<string, string> vars, Encoding encoding = null) 
      => Generate(FileIO.ReadText(path, encoding), vars);

    /// <summary>
    /// Liest ein Template aus einer Datei, ersetzt darin alle Variablen (vars) und speichert das Ergebnis in einer anderen Datei ab.
    /// </summary>
    ...
    {
      if (pathFrom == pathTo)
        throw new NotSupportedException($"{nameof(pathFrom)} und {nameof(pathTo)} dürfen nicht auf die selbe Datei verweisen.");
    ...
    public static void GenerateToFile(string input,
                                      Dictionary<string, string> vars,
                                      string pathTo,
                                      Encoding encoding = null) => FileIO.Write(
      pathTo,
      Generate(input, vars),
      encoding);
  }
