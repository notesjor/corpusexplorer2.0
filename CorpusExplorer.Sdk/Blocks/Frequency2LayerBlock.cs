...
{
  [Serializable]
  public class Frequency2LayerBlock
    : AbstractSimple2LayerBlock,
      ...
      IProvideAggregatedDataItems
  {
    ...
    /// </summary>
    public Dictionary<string, Dictionary<string, double>> Frequency { get; private set; }
      ...
      int[][] doc2)
    {
      ...
      for (var j = 0; j < doc1[i].Length; j++)
      {
        ...
        var wor = layer2[doc2[i][j]];

        Dictionary<string, double> level1;
        lock (_lock1)
        {
          if (!Frequency.TryGetValue(pos, out level1))
          {
            level1 = new Dictionary<string, double>();
