...
namespace CorpusExplorer.Sdk.Extern.QuickIndexRocks
{
  ...
  /// Index wird nur dann geladen wenn WakeUp() aufgerufen wird.
  /// Index wird dann entladen wenn GotToSleep() aufgerufen wird.
  /// Vorteil: Minimiert RAM-Footprint (vertretbare Latenz)
  /// Nachteil: WakeUp() / GoToSleep() müssen aufgerufen werden.
  ...
  public class QuickIndex : IDisposable
  {
    ...
    {
      _dic = DictionaryIndex.ReadDic(pathToCec6, layerDisplayname);
      _dicRes = DictionaryIndex.ReadDicRes(pathToCec6, layerDisplayname);
