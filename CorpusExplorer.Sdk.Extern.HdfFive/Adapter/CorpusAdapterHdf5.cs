...
namespace CorpusExplorer.Sdk.Extern.HdfFive.Adapter
{
  public class CorpusAdapterHdf5 : AbstractCorpusAdapter
  {
    ...
    {
      _path = path;
      _fileId = Hdf5.OpenFile(path);
      _root = Hdf5.CreateOrOpenGroup(_fileId, "/ce");
    }
    ...
    public static AbstractCorpusAdapter Create(Dictionary<Guid, Dictionary<string, object>> documentMetadata, Dictionary<string, object> corpusMetadata, List<Concept> concepts)
    {
      ...
      var path = ""; // TODO path;
      var fileId = Hdf5.OpenFile(path);
      var root = Hdf5.CreateOrOpenGroup(fileId, "/ce");

      var guid = Guid.NewGuid();
      Hdf5.WriteAsciiString(root, "guid", guid.ToString());
    }
    ...
    {
      get => Hdf5.ReadUnicodeString(_root, "name");
      set => Hdf5.WriteUnicodeString(_root, "name", value);
    }

    public override Guid CorpusGuid { get => Guid.Parse(Hdf5.ReadAsciiString(_root, "guid")); }
