      ...
      <v-col cols="9">
        <v-row>
          ...
          <v-col cols="3" style="text-align: right">
            <v-btn @click="$emit('LoadFiles', TableSelection)" color="primary"
            :disabled="TableSelection.length == 0"
              >{{ SingleSelect ? "Datei laden" : this.TableSelection.length === 1 ? "Datei laden" : this.TableSelection.length + " Datein laden" }}</v-btn
            >
          </v-col>
        </v-row>
        <v-data-table
