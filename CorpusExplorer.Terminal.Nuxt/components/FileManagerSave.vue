    ...
    <v-row>
      ...
      <v-col>
        <v-row>
          ...
          label="Neuen Dateinamen eingeben..."
...
export default {
  ...
  methods: {
    ...
    async driveChanged() {
      await this.$axios.$get("http://localhost:12712/fs/set?path=" + this.CurrentDrive);
    ...
    async goToIndex(index){
      var relPath = [this.CurrentDrive.replace(this.DirectorySeparatorChar, ""), ...this.CurrentPath.slice(0, index + 1).map(x=>x.text)].join(this.DirectorySeparatorChar);
      console.log(relPath);
  ...
  watch: {
    ...
    SaveFileName: function(){
      this.$emit('SaveFile', this.SaveFileName);
    }
  }
};
</script>