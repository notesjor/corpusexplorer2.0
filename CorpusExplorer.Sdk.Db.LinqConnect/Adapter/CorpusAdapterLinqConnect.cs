...
using Corpus = CorpusExplorer.Sdk.Db.LinqConnect.Model.Corpus;
...
namespace CorpusExplorer.Sdk.Db.LinqConnect.Adapter
{
  public class CorpusAdapterLinqConnect : AbstractCorpusAdapter
  {
    ...
    public override AbstractCorpusBuilder GetCorpusBuilder() => new CorpusBuilderLinqConnect();
    ...
    {
      var context = new DataContext(LinqConnectConfiguration.ConnectionString);
      var corpus = context.Corpora.First();
      if (corpus == null)
        return null;
    ...
    public override void LayerNew(string layerDisplayname)
    {
      _db.Layers.InsertOnSubmit(
           new CorpusExplorer.Sdk.Db.LinqConnect.Model.Layer
           {
             CorpusID = _corpus.ID,
