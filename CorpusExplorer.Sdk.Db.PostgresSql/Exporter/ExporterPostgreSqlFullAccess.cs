...
using CorpusExplorer.Sdk.Db.PostgreSql.Model.Full;
using CorpusExplorer.Sdk.Model;
using CorpusExplorer.Sdk.Model.Adapter.Corpus.Abstract;
using CorpusExplorer.Sdk.Model.Extension;
using CorpusExplorer.Sdk.Model.Interface;
using CorpusExplorer.Sdk.Utils.DocumentProcessing.Exporter.Abstract;
using Corpus = CorpusExplorer.Sdk.Db.PostgreSql.Model.Full.Corpus;
using Document = CorpusExplorer.Sdk.Db.PostgreSql.Model.Full.Document;
using Layer = CorpusExplorer.Sdk.Db.PostgreSql.Model.Full.Layer;

namespace CorpusExplorer.Sdk.Db.PostgreSql.Exporter
{
  public class ExporterPostgreSqlFullAccess : AbstractExporter
  {
    ...
    public override void Export(IHydra hydra, string path)
    {
      ...
      var context = new FullAccessContext($"data source={path};failifmissing=False");
    ...
    private void AddAnnotations(Dictionary<Guid, int> guids, Dictionary<string, Dictionary<string, ulong>> values, Selection selection, FullAccessContext context)
    {
      ...
      {
        var mdoc = selection.GetReadableMultilayerDocument(dsel.Key).ToDictionary(x => x.Key, x => x.Value.Select(y => y.ToArray()).ToArray());
      ...
      context.SubmitChanges(Devart.Data.Linq.ConflictMode.ContinueOnConflict);
    }
    ...
    private Dictionary<string, Dictionary<string, ulong>> AddLayerValues(Dictionary<string, int> layers, Selection selection, FullAccessContext context)
    {
      ...
      foreach (var layer in selection.Layers)
      {
        ...
        foreach (var x in dict)
        {
          ...
          {
            ID = (ulong)vidx,
            LayerID = layers[layer.Displayname],
            Value = x.Key
          });
        }
      }
      ...
      {
        ID = idx,
        LayerID = layers[SentenceLayerMark],
    ...
    private static void AddDocumentAndDocumentMetadata(Dictionary<Guid, int> guids, Selection selection, FullAccessContext context)
    {
