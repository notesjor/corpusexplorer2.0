...
namespace CorpusExplorer.Sdk.Db.SQLite.Helper
{
  ...
  {
    public static DataTable Read(string path, string query = null, string[] columns = null, string[] dateTimeColumns = null, ulong start = 0, ulong pageSize = 0, bool readOnly = true)
    {
      ...
      using (var connection = new SQLiteConnection($"Data Source={path};Version=3;FailIfMissing=True;{(readOnly ? "Read Only=True;Journal Mode=Off;" : "")}"))
      {
        connection.Open();
    ...
    public static DataTable ReadComplex(string path, string[] columns = null, string[] dateTimeColumns = null, string ownComplexQuery = null, bool readOnly = true)
    {
      var schema = SchemaHelper.ReadSchema(path);
      var dt = BuildDataTable(schema, columns, dateTimeColumns, out var isDateTime);
      var max = isDateTime.Length;
    ...
    private static DataTable BuildDataTable(KeyValuePair<string, Type>[] schema, string[] columns, string[] dateTimeColumns, out bool[] isDateTime)
    {
