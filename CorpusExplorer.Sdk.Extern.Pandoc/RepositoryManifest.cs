  ...
  public class RepositoryManifest : AbstractAddonRepository
  {
    ...
    public override IEnumerable<KeyValuePair<string, AbstractScraper>> AddonScrapers =>
      new Dictionary<string, AbstractScraper>
      {
        ...
        {"PANDOC [markdown - github] (*.txt; *.*)|*.*", new PandocScraper{ Format = PandocScraper.InputFormat.markdown_github} },
        {"PANDOC [markdown - mnd] (*.txt; *.*)|*.*", new PandocScraper{ Format = PandocScraper.InputFormat.markdown_mmd} },
        {"PANDOC [markdown - phpextra] (*.txt; *.*)|*.*", new PandocScraper{ Format = PandocScraper.InputFormat.markdown_phpextra} },
        {"PANDOC [markdown - strict] (*.txt; *.*)|*.*", new PandocScraper{ Format = PandocScraper.InputFormat.markdown_strict} },
        {"PANDOC [wikipedia / mediawiki] (*.txt; *.*)|*.*", new PandocScraper{ Format = PandocScraper.InputFormat.mediawiki} },
