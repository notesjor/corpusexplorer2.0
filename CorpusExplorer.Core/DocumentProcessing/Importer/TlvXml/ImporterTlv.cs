...
namespace CorpusExplorer.Core.DocumentProcessing.Importer.TlvXml
{
  public class ImporterTlv : AbstractImporter
  {
    ...
    private IEnumerable<AbstractCorpusAdapter> Execute(ref XmlDocument xml)
    {
      ...
      var documentMeta = new Dictionary<Guid, Dictionary<string, object>>();
        ...
        foreach (XmlElement x in layers)
        {
          ...
          var dict = (from XmlElement y in x.ChildNodes where y.Name == "value" select y).ToDictionary(y => HttpUtility.HtmlDecode(y.GetAttribute("label")), y => int.Parse(y.GetAttribute("v")));
        ...
        foreach (XmlElement b in docs.ChildNodes)
        {
          ...
          {
            ParseNodeRecursive(ref cache, ref doc, ref sidx, ref widx, n);
          }
          ...
          foreach (var x in ldocs)
          {
            ((LayerValueState)layerstate[x.Key]).AddCompleteDocument(guid, x.Value);
          }
          #endregion
        }
        #endregion      
    ...
    private void ParseNodeRecursive(ref Dictionary<int, KeyValuePair<string, Dictionary<int, string>>> cache, ref Word[][] doc, ref int sidx, ref int widx, XmlNode xmlNode, Dictionary<string, string> values = null)
    {
