  ...
  public class ExporterTlv : AbstractExporter
  {
    ...
    public override void Export(IHydra hydra, string path)
    {
      ...
      foreach (var x in cmeta)
        if (x.Value != null)
          stb.Append($"<entry category=\"{x.Key}\" type=\"{x.Value.GetType()}\">{CleanText(HttpUtility.HtmlDecode(x.Value.ToString()))}</entry>");
      ...
      foreach (var dsel in corpus.DocumentGuids)
      {
        ...
        // Wort-Layer muss verworfen werden.
        var dlayers = corpus.GetLayersOfDocument(dsel).Where(x => x.Displayname != "Wort").ToDictionary(x => x.Displayname, x => x.Guid);
        var d3D = corpus.GetReadableMultilayerDocument(dsel).ToDictionary(x => x.Key, x => x.Value.Select(y => y.ToArray()).ToArray());
        ...
        for (var s = 0; s < wordLayer.Length; s++)
        {
          for (var w = 0; w < wordLayer[s].Length; w++)
          {
            ...
            {
              word.Insert(0, $"<t l=\"{layers[dlayers[otherLayer.Key]]}\" v=\"{lyDict[dlayers[otherLayer.Key]][otherLayer.Value[s][w]]}\">");
              word.Append("</t>");
            }
    ...
    private string CleanText(string input) { return HttpUtility.HtmlEncode(_regex.Replace(input, string.Empty)); }
  }
}
